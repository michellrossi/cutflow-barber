...
export const DashboardPanel: React.FC<{ onNavigate: (tab: any, filter?: string) => void }> = ({ onNavigate }) => {
    ...
    const revenueStats = useMemo(() => {
        ...
        appointments.forEach(apt => {
            ...
            if (aptDate >= startOfToday) todayValue += value;
            if (aptDate >= startOfWeek) weekValue += value;
            if (aptDate >= startOfMonth) monthValue += value;
            if (aptDate >= startOfYear) yearValue += value;
        });

        return { today: todayValue, week: weekValue, month: monthValue, year: yearValue };
    }, [appointments]);
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard
                    ...
                    colorClass="text-emerald-600 bg-emerald-50"
                    label="Lucro da Loja"
                    value={`R$ ${(revenueStats.week * 0.5).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`}
                    trend="Semana"
...
const StatCard: React.FC<StatCardProps> = ({ icon, label, value, trend, subtitle, colorClass, onClick }) => (
    ...
    >
        {/* Header: Ícone + Label alinhados e Trend à direita */}
        ...
        <div className="flex flex-col items-start">
            ...
            {subtitle && <p className="text-[10px] text-slate-400 font-bold uppercase mt-2 tracking-tighter">{subtitle}</p>}
        </div>
    </motion.div>
);
