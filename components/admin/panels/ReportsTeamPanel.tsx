...
export const ReportsTeamPanel: React.FC = () => {
    const { professionals, fetchFinancialReport } = useShop();
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {professionals.map(pro => {
                    const proAppointments = filteredAppointments.filter(a => a.professionalId === pro.id && a.status === 'completed');
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="space-y-2">
                    {professionals
                        .sort((a, b) => {
                            const revA = filteredAppointments.filter(appt => appt.professionalId === a.id && appt.status === 'completed').reduce((acc, appt) => acc + appt.totalValue, 0);
                            const revB = filteredAppointments.filter(appt => appt.professionalId === b.id && appt.status === 'completed').reduce((acc, appt) => acc + appt.totalValue, 0);
                            return revB - revA;
                        })
                        .map((pro, index) => (
                            <div key={pro.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                ...
                                <span className="font-bold text-slate-900">
                                    R$ {filteredAppointments.filter(appt => appt.professionalId === pro.id && appt.status === 'completed').reduce((acc, appt) => acc + appt.totalValue, 0).toFixed(2)}
                                </span>
                            </div>
                        ))}
                </div>
