...
import { SIDEBAR_GROUPS, findGroupForTab, type AdminTab } from './sidebarConfig';

const SIDEBAR_ICONS: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
    LayoutGrid, CalendarCheck, Users, Scissors, UserCircle, CreditCard, Tag,
    Award, BarChart3, Package, TrendingUp, Target, MessageSquare, Sparkles,
...
export const AdminDashboard: React.FC<{ onLogout: () => void, onViewClient: () => void }> = ({ onLogout, onViewClient }) => {
    ...
    // Estado de seções abertas/fechadas na sidebar. Inicializa com os
    // defaults do config (algumas seções já abertas, outras não).
    const [openSections, setOpenSections] = useState<Record<string, boolean>>(() => {
        const initial: Record<string, boolean> = {};
        SIDEBAR_GROUPS.forEach(group => {
            if (group.collapsible) initial[group.id] = group.defaultOpen;
        });
        return initial;
    });
    ...
    useEffect(() => {
        ...
        if (savedSections) {
            try {
                ...
                // valor corrompido, ignora e mantém defaults
            }
        }

        // Independente da preferência salva, a seção da tab ativa precisa
        // estar aberta — senão o item selecionado fica escondido ao recarregar.
        if (savedTab) {
            const groupId = findGroupForTab(savedTab as AdminTab);
            if (groupId) setOpenSections(prev => ({ ...prev, [groupId]: true }));
        }
    ...
    useEffect(() => {
        localStorage.setItem('adminSidebarSections', JSON.stringify(openSections));
    }, [openSections]);
    ...
    const handleTabChange = (tab: AdminTab, filter?: string) => {
        setActiveTab(tab);
        const groupId = findGroupForTab(tab);
        if (groupId) {
            setOpenSections(prev => (prev[groupId] ? prev : { ...prev, [groupId]: true }));
        }
    ...
    return (
        <div className="flex h-screen bg-white text-slate-900 overflow-hidden flex-col md:flex-row w-full">
            ...
            >
                ...
                <nav className="flex-1 px-2 py-2 space-y-0.5 no-scrollbar overflow-y-auto">
                    {SIDEBAR_GROUPS.map(group => (
                        <div key={group.id} className={group.title ? 'mt-3 first:mt-0' : ''}>
                            {group.title && isSidebarExpanded && (
                                <button
                                    onClick={() => group.collapsible && toggleSection(group.id)}
                            ...
                            {(!group.collapsible || !group.title || openSections[group.id] || !isSidebarExpanded) && (
                                <div className="space-y-0.5">
                                    {group.items.map(item => {
                                        const Icon = SIDEBAR_ICONS[item.iconName];
                                        return (
                                            <SidebarItem
                                                key={item.tab}
                                                icon={<Icon size={18} />}
