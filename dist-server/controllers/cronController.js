...
export async function runCronLogic() {
    ...
    const maxRetries = 3;
    const serviceCache = new Map(); // shopId -> (serviceId -> name)
    ...
    if (apts24h) {
        for (const apt of apts24h) {
            ...
            if (!(await isInstanceConnected(apt.shop_id, shop.whatsapp_instance)))
                continue;
    ...
    if (currentHourSP >= 9) {
        ...
        if (bdayError)
            console.error('[Cron] Erro bday RPC:', bdayError.message);
        if (bdayClients && bdayClients.length > 0) {
            ...
            for (const client of clients) {
                ...
                if (!shop?.whatsapp_connected)
                    continue;
                if (!(await isInstanceConnected(client.shop_id, shop.whatsapp_instance)))
                    continue;
        ...
        if (now.hour() === 7 && now.minute() < 11) {
            ...
            if (allApts && allApts.length > 0) {
                ...
                appointments.forEach(apt => {
                    ...
                    if (apt.date >= sevenDaysAgo)
                        shop.currentWeek.push(apt);
                    else
                        shop.prevWeek.push(apt);
                });
