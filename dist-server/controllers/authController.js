...
export const requestClientLogin = async (req, res) => {
    try {
        const { shopId, phone, name, birthDate, justCheck } = req.body;
        if (!shopId || !phone)
            return res.status(400).json({ error: 'ShopId e Telefone são obrigatórios' });
        const secret = getJwtSecret();
        if (!secret)
            return res.status(500).json({ error: 'Erro de configuração no servidor (JWT_SECRET)' });
        const cleanPhone = phone.replace(/\D/g, '');
        let { data: client } = await supabaseAdmin.from('clients').select('*').eq('shop_id', shopId).eq('phone', cleanPhone).maybeSingle();
        if (!client) {
            if (justCheck && cleanPhone !== '11999999999')
...
export const validateClientToken = async (req, res) => {
    try {
...
export const triggerWelcomeEmail = async (req, res) => {
    try {
        const { email, name, shopId } = req.body;
        if (!email || !name || !shopId)
