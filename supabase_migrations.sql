...
-- PROFESSIONALS: comissão, cor e telefone
...
-- APPOINTMENTS: flags de notificação e controle de tentativas
...
-- Script 5: colunas target e category em message_templates
...
-- Tokens de Autenticação via WhatsApp
...
CREATE POLICY "Visibilidade Publica Lojas" ON public.shops
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Dono_Gere_Loja" ON public.shops;
CREATE POLICY "Dono_Gere_Loja" ON public.shops
  FOR ALL USING (auth.uid() = owner_id);

-- ---- SERVICES ----
DROP POLICY IF EXISTS "Publico_Ve_Servicos" ON public.services;
CREATE POLICY "Publico_Ve_Servicos" ON public.services
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Dono_Gere_Servicos" ON public.services;
CREATE POLICY "Dono_Gere_Servicos" ON public.services
  FOR ALL USING (
...
CREATE POLICY "Publico_Ve_Profissionais" ON public.professionals
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Dono_Gere_Profissionais" ON public.professionals;
CREATE POLICY "Dono_Gere_Profissionais" ON public.professionals
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.shops WHERE id = professionals.shop_id AND owner_id = auth.uid())
  );

-- ---- APPOINTMENTS ----
DROP POLICY IF EXISTS "Dono_Gere_Agendamentos" ON public.appointments;
CREATE POLICY "Dono_Gere_Agendamentos" ON public.appointments
  FOR ALL USING (
...
CREATE POLICY "Criar Agendamento Paywall" ON public.appointments
  FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.shops WHERE id = appointments.shop_id AND plan IN ('active', 'trial'))
  );

-- Script 7 (versão final): SELECT restrito a donos autenticados.
-- O backend usa supabaseAdmin (ignora RLS) para leitura pública de disponibilidade.
...
-- ---- CLIENT_AUTH_TOKENS ----
-- Script 6 (versão final): bloqueia qualquer acesso via anon key.
-- supabaseAdmin ignora esta policy. Tokens nunca ficam expostos publicamente.
DROP POLICY IF EXISTS "Tokens_Acesso_Publico" ON public.client_auth_tokens;
DROP POLICY IF EXISTS "Public can view active tokens" ON public.client_auth_tokens;
DROP POLICY IF EXISTS "Leitura segura de tokens" ON public.client_auth_tokens;
...
CREATE POLICY "Dono_Gere_Templates" ON public.message_templates
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.shops WHERE id = message_templates.shop_id AND owner_id = auth.uid())
  );
...
-- Marca como enviados agendamentos antigos presos em loop de reagendamento:
...
-- Marca pós-venda antigos como enviados para evitar loop:
-- UPDATE public.appointments
